    ...
    with open(options.output, "w") as f:
        ...
        for test_id, test_times in tests_by_id.items():
            ...
            for runner in runners:
                ...
                average_time = get_average_time(test_time_list)
                highest_time = get_highest_time(test_time_list)
                lowest_time = get_average_time(test_time_list)
                if options.error and options.repeat > 1:
                    # if preserve error is true, then include lowest and highest times
                    each_runner_time.append(average_time + "-" + lowest_time + "-" + highest_time)
                else:
                    each_runner_time.append(average_time)
            f.write(test_id + "," + ",".join(each_runner_time) + "\n")
    return
